...
function renderChatList() {
  ...
  /* 单聊 + 群聊合并，按最后一条消息时间排序 */
  const singleItems = liaoChats.map((chat, idx) => {
    const role = liaoRoles.find(r => r.id === chat.roleId);
    if (!role) return null;
    const msgs    = (chat.messages || []).filter(m => !m.hidden);
    const lastMsg = msgs.length ? msgs[msgs.length - 1] : null;
    return { type: 'single', idx, chat, role, lastMsg };
  }).filter(Boolean);

  const groupItems = liaoGroups.map((group, idx) => {
    const msgs    = (group.messages || []).filter(m => !m.hidden);
    const lastMsg = msgs.length ? msgs[msgs.length - 1] : null;
    return { type: 'group', idx, group, lastMsg };
  });

  const allItems = [...singleItems, ...groupItems].sort((a, b) => {
    const ta = a.lastMsg ? (a.lastMsg.ts || 0) : 0;
    const tb = b.lastMsg ? (b.lastMsg.ts || 0) : 0;
    return tb - ta;
  });
  ...
  allItems.forEach(item => {
    if (item.type === 'single') {
      const { idx, chat, role, lastMsg } = item;
      let preview = '暂无消息';
      if (lastMsg) {
      ...
      el.innerHTML =
        ...
        '</div>' +
        '<div class="chat-item-meta"><div class="chat-item-time">' + lastTime + '</div></div>';
      el.addEventListener('click', () => openChatView(idx));
      list.appendChild(el);

    } else {
      const { idx, group, lastMsg } = item;
      let preview = '暂无消息';
      if (lastMsg) {
        if (lastMsg.recalled)              preview = '[撤回了一条消息]';
        else if (lastMsg.type === 'image') preview = '[图片]';
        else {
          const senderName = lastMsg.roleName ? lastMsg.roleName + '：' : (lastMsg.role === 'user' ? '我：' : '');
          preview = senderName + (lastMsg.content || '暂无消息');
        }
      }
      const lastTime = lastMsg ? formatTime(lastMsg.ts) : '';
      const el = document.createElement('div');
      ...
      /* 群头像拼合 */
      const avatarHtml = buildGroupAvatarHtml(group.memberIds);

      el.innerHTML =
        '<div class="group-avatar-grid">' + avatarHtml + '</div>' +
        '<div class="chat-item-body">' +
          '<div class="chat-item-name">' + escHtml(group.name || '群聊') +
            '<span class="group-member-count">' + (group.memberIds || []).length + '人</span>' +
          '</div>' +
          '<div class="chat-item-preview">' + escHtml(preview.slice(0, 40)) + '</div>' +
        '</div>' +
        '<div class="chat-item-meta"><div class="chat-item-time">' + lastTime + '</div></div>';
      el.addEventListener('click', () => openGroupView(idx));
      list.appendChild(el);
    }
  });
}
...
/* 群头像拼合HTML */
function buildGroupAvatarHtml(memberIds) {
  ...
  if (!members.length) {
    return '<img src="' + defaultAvatar() + '" class="group-avatar-single" alt="">';
  }
  if (members.length === 1) {
    return '<img src="' + escHtml(members[0].avatar || defaultAvatar()) + '" class="group-avatar-single" alt="">';
  }
  return members.map(m =>
    '<img src="' + escHtml(m.avatar || defaultAvatar()) + '" class="group-avatar-cell" alt="">'
  ).join('');
}

/* ---------- 底部操作栏 ---------- */
document.getElementById('liao-btn-new-role').addEventListener('click', openNewRoleModal);
document.getElementById('liao-btn-new-group').addEventListener('click', openNewGroupModal);
...
function openNewGroupModal() {
  document.getElementById('liao-group-name').value = '';

  /* 渲染成员选择列表 */
  const list = document.getElementById('liao-group-member-list');
  list.innerHTML = '';
  liaoRoles.forEach(role => {
    const item = document.createElement('label');
    item.className = 'group-member-pick-item';
    item.innerHTML =
      '<input type="checkbox" class="group-member-checkbox" value="' + escHtml(role.id) + '">' +
...
document.getElementById('liao-group-confirm').addEventListener('click', () => {
  ...
  if (memberIds.length < 2) { alert('请至少选择2名成员'); return; }
  ...
  const group = {
    ...
    memberIds,
    messages:       [],
    memory:         { longTerm: [], shortTerm: [], important: [], other: {} },
    chatUserName:   liaoUserName,
    chatUserAvatar: liaoUserAvatar,
    chatUserSetting: '',
    chatSettings:   {
      scheduleEnabled: false,
      autoTrigger: { enabled: false, times: [], interval: 0 }
    }
  };

  liaoGroups.push(group);
